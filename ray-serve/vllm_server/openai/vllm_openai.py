...
class ModelApp(ModelAppInterface):
    ...
    def load_chat_template(self, args, tokenizer):
        if args.chat_template is not None:
            ...
            self.logger.info(f"Using default chat template:\n{tokenizer.chat_template}")
        else:
            self.logger.warning("No chat template provided. Chat API will not work.")
    ...
    async def create_chat_completion(
        ...
        """
        error_check_ret = await self.check_model(request)
        ...
        try:
            ...
            self.logger.error(f"Error in applying chat template from request: {str(e)}")
            return self.create_error_response(HTTPStatus.BAD_REQUEST, str(e))

        token_ids, error_check_ret = await self.check_length(request, prompt=prompt)
        if error_check_ret is not None:
            return error_check_ret
    ...
    async def create_completion(self, request: CompletionRequest, raw_request: Request):
        ...
        if use_token_ids:
            _, error_check_ret = await self.check_length(request, prompt_ids=prompt)
        else:
            token_ids, error_check_ret = await self.check_length(request, prompt=prompt)
        if error_check_ret is not None:
            return error_check_ret
